...
// Don't have to allocate the memory here.
...
void hungarian_print_matrix_int(int** C, int rows, int cols) {
    int i,j;
    fprintf(stderr , "\n");
    for(i=0; i<rows; i++) {
        fprintf(stderr, " [");
...
void hungarian_print_assignment(hungarian_problem_t* p) {
  hungarian_print_matrix_int(p->assignment, p->num_rows, p->num_cols) ;
}
...
int hungarian_init(hungarian_problem_t* p, double** cost_matrix, int rows, int cols, int mode) {
  ...
  for(i=0; i<p->num_rows; i++) {
    ...
    for(j=0; j<p->num_cols; j++) {
      p->cost[i][j] =  (i < org_rows && j < org_cols) ? cost_matrix[i][j] : 0.0;
      p->assignment[i][j] = 0;
...
///// no idea why they need this check --- move to other Hungarian cpp or call matlab. ToDo.
/*
