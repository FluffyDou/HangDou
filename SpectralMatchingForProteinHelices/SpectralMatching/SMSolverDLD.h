...
public:
    ...
    void IterativePicking( const ScoreMatrixDoubleLineDir & scoreMatrix );

    // Try picking more than one assignment each iteration
    void IterativePickingNewStop( const ScoreMatrixDoubleLineDir & scoreMatrix, double eigenRatio = 1.0 );

    // Simple greedy method to binarize the result
    void SimpleGreedyPick( const ScoreMatrixDoubleLineDir & scoreMatrix );

    // Compute the score sum for Score Function
    void ComputeFunScore( const ScoreMatrixDoubleLineDir & scoreMatrix );
    ...
    MatrixXd GetFullMatrix(const ScoreMatrixDoubleLineDir & scoreMatrix);
    // Return the indicator Vector
    VectorXd GetIndicatorVec(const ScoreMatrixDoubleLineDir & scoreMatrix);
...
private:
    ...
    {
        int i     = index / (2*moduleLen);
        //int j     = ( index % (2*moduleLen) ) % moduleLen;
    ...
    // Use the first element to compare
    //struct pairData
    //{ 
    //    double first; 
    //    int    second;
    ...
    //static bool IndexCompare(const pairData & a, const pairData & b) {return a.first > b.first;}
