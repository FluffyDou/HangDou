...
void SMSolverSLD::ComputeFunScore( const ScoreMatrixSingleLineDir & scoreMatrix )
{
...
void SMSolverSLD::SolveForEigenvectors( const MatrixXd & matrix )
{
...
void SMSolverSLD::SimpleGreedyPick( const ScoreMatrixSingleLineDir & scoreMatrix )
{
...
void SMSolverSLD::IterativePicking(const ScoreMatrixSingleLineDir & scoreMatrix)
{
    // Make a copy of the score matrix since we need to prune it. 
    MatrixXd matrix = scoreMatrix.GetMatrix();
...
void SMSolverSLD::IterativePickingNewStop(const ScoreMatrixSingleLineDir & scoreMatrix, double eigenRatio)
{
    // Make a copy of the score matrix since we need to prune it. 
    MatrixXd matrix = scoreMatrix.GetMatrix();
...
// Do Simple Greedy on double line direction score matrix
void SMSolverSLD::HungarianPick( const ScoreMatrixSingleLineDir & scoreMatrix )
{
    // Make a copy of the score matrix since we need to prune it. 
    MatrixXd matrix = scoreMatrix.GetMatrix();
...
void SMSolverSLD::PruneMatrix(MatrixXd & matrix, std::vector<bool> & remainIndex, std::vector<bool> & remainIndexOld)
{    
...
// Pad the eigenvector back to original length
void SMSolverSLD::PadEigenvec(const VectorXd & prunedVec, std::vector<double> & paddedEigenvec, const std::vector<bool> & remainIndex)
{
...
void SMSolverSLD::ExcludeAssigns(int pickedIndex, std::vector<bool> & remainIndex, int moduleLen, int dataLen)
{
    // Pick out i,j (fi,ej)
...
void SMSolverSLD::ShrinkMatrix(int largestEleIndex, MatrixXd & matrix, std::vector<bool> & remainIndex, std::vector<bool> & remainIndexOld)
{   
    ...
    for(int i = 0; i < picked; ++i)
        matrix.diagonal()[i] += matrix.row(picked)[i];
    for(int i = picked; i < matrix.cols(); ++i)
        matrix.diagonal()[i] += matrix.col(picked)[i];
...
MatrixXd SMSolverSLD::GetFullMatrix(const ScoreMatrixSingleLineDir & scoreMatrix)
{
    MatrixXd tempMatrix = scoreMatrix.GetMatrix();
    tempMatrix.triangularView<Eigen::Upper>() = tempMatrix.transpose();
...
VectorXd SMSolverSLD::GetIndicatorVec(const ScoreMatrixSingleLineDir & scoreMatrix)
{
    const int mLen = scoreMatrix.GetModuleNum();
    const int dLen = scoreMatrix.GetDataNum();
