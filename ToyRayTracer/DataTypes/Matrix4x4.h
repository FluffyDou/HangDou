...
public:
	...
	// Print out the contents of this matrix to a file pointer
	void OutputMatrix( FILE* file );
	...
	static Matrix4x4 Rotate   ( float angle, const vec3& axis );		 // Not optimized for speed!
	static Matrix4x4 Perspective  ( float fovy, float aspect, float zNear, float zFar );   // Not optimized for speed!
	static Matrix4x4 LookAt       ( const vec3 &eye, const vec3 &at, const vec3 &up );    // Not optimized for speed!

	// Static methods for defining matrices given from outer products (of real-valued vectors)
	static Matrix4x4 OuterProduct ( const vec3& u );					// Returns u * u^T
	static Matrix4x4 OuterProduct ( const vec4& u );					// Returns u * u^T
	static Matrix4x4 OuterProduct ( const vec4& u, const vec4& v );		// Returns u * v^T
