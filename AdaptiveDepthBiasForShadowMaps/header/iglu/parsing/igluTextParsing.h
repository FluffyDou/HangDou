...
#ifndef IGLU_TEXTPARSING_H
#define IGLU_TEXTPARSING_H
...
// Works the same ways as StripLeadingNumber, (basically stripping the leading
// token), doing minor cleaning before returning the token to ensure commas, parens,
// and other formatting junk doesn't get grouped in with a number.
char *StripLeadingNumericalToken( char *string, char *result );

// Selects the next entry of a CSV file (e.g., separated by commas; handles
//    double-quoted entries [allowing commas in entries])
char *StripLeadingCSVEntry( char *string, char *result );
