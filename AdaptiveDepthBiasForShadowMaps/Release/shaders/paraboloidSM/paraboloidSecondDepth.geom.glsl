...
void main( void )
{
	...
	{ 
        ...
        smTexCoord  = vec3( gl_PositionIn[0].xy, 1.0);
		EmitVertex();
		gl_Position = vec4( gl_PositionIn[2].xy, vertTexCoord[2].z, 1.0 ); 
        smTexCoord  = vec3( gl_PositionIn[2].xy, 1.0);
		EmitVertex();
		gl_Position = vec4( gl_PositionIn[1].xy, vertTexCoord[1].z, 1.0 ); 
        smTexCoord  = vec3( gl_PositionIn[1].xy, 1.0);
		EmitVertex();
		EndPrimitive();	
	}
	...
	if ( neg )
	{
        ...
        smTexCoord  = vec3( gl_PositionIn[0].zw, 0.0);
		EmitVertex();
		gl_Position = vec4( gl_PositionIn[1].zw, vertTexCoord[1].z, 1.0 ); 
        smTexCoord  = vec3( gl_PositionIn[1].zw, 0.0);
		EmitVertex();
		gl_Position = vec4( gl_PositionIn[2].zw, vertTexCoord[2].z, 1.0 ); 
        smTexCoord  = vec3( gl_PositionIn[2].zw, 0.0);
		EmitVertex();
		EndPrimitive();
	}
