...
class CAlphaReferenceViewer(CAlphaViewer):
    ...
    def visualizePairwiseDistance(self):
        ...
        # For some reason, the same color can't be displayed more than once. usedColors ensures that
        # the each color is only used once. If an atom should have the same color as a prior atom,
        # we offset the atom's color by a small value that can't be visually distinguished.
        usedColorsRef = []
        usedColorsModel = []
                ...
                if i in chain.residueList:
                    for atomName in chain[i].getAtomNames():
                        ...
                        if atom and atom.getName() == "CA" and atom is not None:
                            ...
                            red, green, blue = self.shiftColor(usedColorsRef, int(red), int(green), int(blue))
                ...
                if i in chain.residueList:
                    for atomName in chain[i].getAtomNames():
                        ...
                        if atom and atom.getName() == "CA" and atom is not None:
                            ...
                            red, green, blue = self.shiftColor(usedColorsModel, int(red), int(green), int(blue))
    ...
    # Helper function that shifts a color's RGB values slightly until it is
    # a unique color in a list of colors
    def shiftColor(self, usedColors, red, green, blue):
        while (red, green, blue) in usedColors:
            if red is not 255: red += 1
            if green is not 255: green += 1
            if blue is not 255: blue += 1
        usedColors.append((red,green,blue))
