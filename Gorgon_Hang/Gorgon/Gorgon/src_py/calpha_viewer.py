...
#   Revision 1.66  2012/06/18 13:27:00  burrowsd
...
class CAlphaViewer(BaseViewer):
   ...
   # Overridden
    ...
    def loadFixedData(self, inputFileName, chainID):
        #Overwriting the function in BaseViewer
        def setupChain(mychain):
            self.main_chain = mychain
            self.loadedChains.append(mychain)
        ...
        if filename.split('.')[-1].lower() == 'pdb':
            if inputFileName:
                ...
                if self.whichChainID == 'ALL':
                    mychainKeys = Chain.loadAllChains(inputFileName, qparent=self.app)
                    for chainKey in mychainKeys:
                        setupChain(Chain.getChain(chainKey))
                else:
                    mychain = Chain.load(inputFileName, qparent=self.app, whichChainID = self.whichChainID)
                    setupChain(mychain)
...
starting residue number).
        ...
        if not self.fileName.isEmpty():
            ...
            self.dirty = False
            self.setCursor(QtCore.Qt.ArrowCursor)
    def saveFixedData(self, outputName):
